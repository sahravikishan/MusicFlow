...
const GUITAR_STRINGS = ['E1', 'A1', 'D2', 'G2'];
...
// Fixed: Added function to compute total duration in seconds based on beats and BPM for consistent timing displays and playback progress
...
function handlePatternChange() {
    ...
    if (!select) return;
    selectedStrummingPattern = select.value;
    const cont = document.getElementById('customPatternContainer');
    if (selectedStrummingPattern === 'Custom') {
...
async function generateAudio() {
    ...
    // Fixed: Compute totalDuration in seconds using BPM for accurate playback timing and display
...
function downloadAudio() {
    showToast('Download feature (mock): bass-composition-bass.wav');
}
...
function updateAnalysis() {
    ...
    // Fixed: Compute beats and seconds separately for accurate display in beats and time
    ...
    if (patternDisplay) patternDisplay.textContent = selectedStrummingPattern
        ? (selectedStrummingPattern === 'Custom' ? customPattern || 'Custom Pattern' : selectedStrummingPattern)
        : 'N/A';
...
function updateStats() {
    ...
    // Fixed: Use getTotalSeconds() for accurate time display in seconds
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('✅ Bass JS Loaded');

    // Wire inputs once
    const strumSelect = document.getElementById('strummingPattern');
    if (strumSelect) strumSelect.addEventListener('change', handlePatternChange);
